...
输出完整的 Markdown 格式文章，包含 frontmatter (title, description, pubDate, category, keywords, image)。`;
  ...
  // ========== Pinterest Agent 模块 ==========
const PinterestAgent = require('./pinterest-agent.cjs');

  async runPinterestAgent() {
    const weekNum = this.getWeekNumber();
    
    // 使用专门的 Pinterest Agent 生成内容
    const pinterest = new PinterestAgent();
    const result = await pinterest.run(weekNum);
    
    console.log(`    [M-PINTEREST] 生成 ${result.pinterest.summary.totalPins} 条 Pin 文案`);
    console.log(`    [M-INSTAGRAM] 生成 ${result.instagram.length} 条 IG 帖子`);
    
    return {
      pinterestFile: result.files[0],
      instagramFile: result.files[1],
      summary: result.pinterest.summary
    };
  }
